...
public class GithubStorageFactory implements PreferenceStorageFactory {
    ...
    public boolean canOpen(URL location) {
    	try {
    		return PROTOCOL.equalsIgnoreCase(location.getProtocol()) && HOST.equalsIgnoreCase(location.getHost()) && Pattern.matches(SLASH_PLUS_HEX_DIGITS_REGEXP, location.getPath());
